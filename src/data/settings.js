...
export const customizeConfig = ({ options, updateOption }) => ({
  ...
  3: {
    name: 'View Density',
    desc: 'Adjust card size and columns on Apps and Games.',
    config: densityConfig,
    value: find(densityConfig, (c) => c.value?.viewDensity === (options.viewDensity || 'default'), 1),
    type: 'select',
    action: (a) => updateOption(a),
  },
  4: {
...
export const advancedConfig = ({ options, updateOption }) => ({
  1: {
    ...
    desc: 'Disable animated background effects to improve performance.',
    value: options.performanceMode ?? false,
    type: 'switch',
